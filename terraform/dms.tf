resource "aws_dms_replication_instance" "dms_engine" {
  replication_instance_id       = "jgs-dms-instance"
...
resource "aws_dms_endpoint" "source_postgres" {
  ...
  depends_on = [aws_db_instance.ecommerce_db]
...
resource "aws_dms_endpoint" "target_s3" {
  ...
  s3_settings {
    # Referenciando especificamente o bucket 'bronze' do for_each
    bucket_name             = aws_s3_bucket.datalake_layers["bronze"].bucket
    bucket_folder           = "olist_dms_export" 
    data_format             = "parquet"          
